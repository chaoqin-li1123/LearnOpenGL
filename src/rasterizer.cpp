...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    float x1 = v[0][0], y1 = v[0][1]; 
    float x2 = v[1][0], y2 = v[1][1]; 
    float x3 = v[2][0], y3 = v[2][1]; 
    ...
    float dx[4]{0.0, 0.0, 0.5, 0.5};
    float dy[4]{0.0, 0.5, 0.0, 0.5};
    for (int x0 = xmin; x0 <= xmax; x0++) {
        for (int y0 = ymin; y0 <= ymax; y0++) {
            int cnt = 0;
            for (int k = 0; k < 4; k++) {
                float x = x0 + dx[k], y = y0 + dy[k];
                auto [alpha, beta, gamma] = computeBarycentric2D(x, y, t.v);
                if (alpha < 0.0 || beta < 0.0 || gamma < 0.0) continue;
                float w_reciprocal = 1.0/(alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                float z_interpolated = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                z_interpolated *= w_reciprocal;
                int idx = get_index(x * 2, y * 2);
                if (depth_buf[idx] < z_interpolated) continue;
            ...
            set_pixel(Eigen::Vector3f{(float)x0, (float)y0, 0}, factor * color);
        }
    }
